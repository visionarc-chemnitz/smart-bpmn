...
def predict_object(image: ndarray, object_predictor: ObjectPredictor) -> List[ObjectPrediction]:
    ...
    print(f"Inference time for object prediction: {inference_time:.4f} seconds")

    return [ObjectPrediction(label, *box) for box, label in predictions]


def predict_keypoint(image: ndarray, keypoint_predictor: KeyPointPredictor) -> List[KeyPointPrediction]:
    ...
    inference_time = time.time() - start_time
    print(f"Inference time for keypoint prediction: {inference_time:.4f} seconds")
