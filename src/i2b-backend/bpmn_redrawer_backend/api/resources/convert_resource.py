...
async def convert_image(request: Request):
    ...
    ocr_img, predict_img = ss.get_ocr_and_predict_images(image)
    ...
    if "elements" in form and form["elements"] == 'true':
        obj_predictions = ps.predict_object(predict_img, request.app.state.object_predictor)
        elements = cs.convert_object_predictions(obj_predictions)

        if "flows" in form and form["flows"] == 'true':
            kp_predictions = ps.predict_keypoint(ocr_img, request.app.state.keypoint_predictor)
